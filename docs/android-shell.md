...
- the Android shell CI build now stamps each workflow run with a monotonically increasing APK `versionCode` from `github.run_number` and a commit-tagged `versionName`, so the phone installer can compare installed vs downloaded shell versions reliably across pushes
