...
- **Built-In Observability Expansion**: Added persistent policy-audit records, provider-health snapshots, request-latency tracking, planner execution timelines, filtered proxy-log queries, candidate test-output retrieval, and Mode B Android diagnostics for battery, logcat, process-memory, and thermal captures. The dashboard now surfaces these diagnostics directly through dedicated operator panels.
...
- The portable phone installer now configures `llama.cpp` with `GGML_NATIVE=OFF` and `LLAMA_OPENSSL=OFF` so proot builds do not waste time on failing `-mcpu=native` ARM feature probes or optional OpenSSL discovery for disabled HTTPS paths.
