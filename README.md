...
- The Android shell architecture docs now include `docs/android-shell-refactor-plan.md`, which records the proposed `ShellCoordinator` and `StartupOrchestrator` split, the duplicated voice-session responsibilities across the shell services, and the smallest safe extraction path
- The Android shell now shares one `VoiceSessionController` between the foreground service and accessibility overlay path, separates backend HTTP transport from Termux runtime command dispatch behind a temporary `BackendSupervisor` facade, and now also ships a real `ShellCoordinator`, `ShellState`, and `StartupOrchestrator` so Android shell surfaces consume one shared runtime/status snapshot instead of recomputing state independently
...
At the end of install, the script now runs a real smoke check using the shipped lifecycle scripts. If the backend is already running, the smoke check reuses that process instead of stopping it. It verifies `/api/status`, checks `/api/runtime/provider-registry`, `/api/runtime/tool-manifest`, and `/api/voice/engines` for the expected runtime contracts, verifies `llama-cli`, `whisper`, and `piper` are visible on `PATH` when local model runtimes are enabled, and only stops the backend when the smoke check had to start it itself. The result is written as JSON to `$HOME/mordecai/data/logs/install-verification-report.json`, failed runs include the tail of `$HOME/mordecai/data/logs/backend.log`, and every report now carries a UTC timestamp plus the installer version and checkout commit for correlation across reruns.

The shell APK update path is now version-aware. On reinstall, the script queries the installed `ai.mordecai.shell` package first, inspects the downloaded APK version with `aapt` when available, falls back to comparing the installed APK file hash against the downloaded APK hash, and only launches the Android install flow when the APK is newer or otherwise different.
