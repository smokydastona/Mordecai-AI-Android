...
def test_phase1_installer_provisions_phone_supported_local_model_runtimes():
    ...
    assert "'${ENV_DIR}/bin/python' -m pip install --upgrade pip 'setuptools<82' wheel" in installer
...
def test_phase1_installer_runs_post_install_backend_smoke_check():
    ...
    assert "assert tool_manifest['contract'] == 'tool-manifest'" in installer
    assert "assert tool_manifest['tool_count'] == len(tool_manifest['tools'])" in installer
    assert "assert 'catalog_summary' in voice_engines" in installer
    assert "assert 'recommended_stack' in voice_engines" in installer
    assert 'Backend status probe failed at ${status_url} with exit code ${curl_status}.' in installer
    assert 'Runtime API contract probe failed for provider registry, tool manifest, or voice engines.' in installer
    assert 'Managed local runtime commands were not all visible on PATH inside the runtime.' in installer
